...
let currentPersonaFull = null;      // {id, avatar, title_cn, profile_summary, personality_summary, attributes}
...
async function loadPersonas() {
  ...
  await loadPersonaFull(currentPersonaId);
...
async function loadPersonaFull(pid) {
  const r = await fetch(`api/persona/${pid}`);
  currentPersonaFull = await r.json();
  unlockedKeys.clear();
  renderAttrGrid(currentPersonaFull);
  $("#persona-title-inline").textContent =
    `${currentPersonaFull.avatar} ${currentPersonaFull.title_cn}`;
  updateMatchSummary();
}
...
function unlockAttrs(keys) {
  ...
  keys.forEach((k) => {
    ...
    // remove flash class after animation so next round can re-flash if needed
...
function updateMatchSummary() {
  const total = currentPersonaFull ? (currentPersonaFull.attributes || []).length : 0;
  const got = unlockedKeys.size;
  $("#match-summary").textContent = `${got} / ${total}`;
  // VS 对比里 PAMDP 侧也同步该值
  $("#vs-cover-p").textContent = `${got} / ${total}`;
  // Vanilla 永远是 0：基线 LLM 不做主动 persona 推断
  $("#vs-cover-v").textContent = `0 / ${total}`;
}

// ============ KPI · PAMDP vs Vanilla ============
...
function updateKpi(t) {
  // —— 主对比：累计回报 ——
  const rewardP = $("#vs-reward-p");
  const rewardV = $("#vs-reward-v");
  ...
  // —— 主对比：胜率 ——
  const winP = $("#vs-winrate-p");
  const winV = $("#vs-winrate-v");
  ...
  // —— 主对比：画像覆盖 —— (覆盖在 unlockAttrs 之后由 updateMatchSummary 统一刷新)

  // —— footer 论文公式指标 ——
  $("#kpi-turn").textContent = `${t.turn} / 6`;
...
function clearAll() {
  ...
  // 主 VS 对比
  $("#vs-reward-p").textContent = "+0.0";
  $("#vs-reward-v").textContent = "0.0";
  $("#vs-winrate-p").textContent = "0%";
  $("#vs-winrate-v").textContent = "0%";
  // footer
  $("#kpi-turn").textContent = "0 / 6";
  $("#kpi-adv").textContent = "+0.000";
  $("#kpi-adv").classList.remove("good", "bad");
  $("#kpi-vh").textContent = "0.00 / 0.00";
  // reset attribute cards
  unlockedKeys.clear();
  ...
  try {
    ...
    console.warn("[PAMDP] chart reset failed (continuing):", e);
  }
}
...
function pushChartPoint(t) {
  try {
    ...
    console.warn("[PAMDP] pushChartPoint failed (continuing):", e);
  }
}
...
async function doStep() {
  ...
  try {
    ...
    if (!r.ok) { status(`✗ HTTP ${r.status}`, "bad"); inProgress = false; return; }
    const data = await r.json();
    if (data.error) { status("✗ " + data.error, "bad"); inProgress = false; return; }
    if (data.done) {
      status(`✓ 演示结束（共 ${data.turn} 轮）`, "accent");
...
async function doStart() {
  console.log("[PAMDP] doStart clicked");
  try {
    clearAll();
    const persona_id = $("#persona-select").value;
    const mode = $("#mode-select").value;
    status("⏳ 初始化…", "");
    const r = await fetch("api/start", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ persona_id, mode }),
    });
    if (!r.ok) { status(`✗ HTTP ${r.status}`, "bad"); return; }
    const data = await r.json();
    if (data.error) { status("✗ " + data.error, "bad"); return; }
    // 切换画像时同步刷新属性卡片
    if (persona_id !== currentPersonaId) {
      currentPersonaId = persona_id;
      await loadPersonaFull(currentPersonaId);
    }
    status(`✓ Episode ready · ${data.mode} mode · max=${data.max_turn}`, "accent");
    $("#step-btn").disabled = false;
    $("#auto-btn").disabled = false;
    $("#reset-btn").disabled = false;
    $("#start-btn").textContent = "▶ 重新开始";
  } catch (err) {
    console.error("[PAMDP] doStart error:", err);
    status("✗ " + (err && err.message ? err.message : err), "bad");
  }
}
...
function cssEscape(s) {
  // 简易转义：属性 key 只用了字母/数字/下划线，足够
  return String(s).replace(/[^a-zA-Z0-9_-]/g, "\\$&");
}

// ============ boot ============
function boot() {
  console.log("[PAMDP] boot");
